...
- **시황분석**(tab 1, `src/client/pages/MarketPage.tsx`): 시장 지표 대시보드 **✅ 구현 완료**. 하루 1회 배치 수집(기본 07시 KST `MARKET_HOUR`) → `settings` KV(`key="marketSnapshot"`, 마이그레이션 불필요)에 JSON 스냅샷 저장. tRPC `market.latest`(저장본)·`market.refresh`('지금 갱신' 버튼=즉시 재수집). 서버 수집기 = **`src/server/market/`**: `cnn.ts`/`tradingview.ts`/`adr.ts`/`index.ts`(병렬·소스별 tolerant — 한 곳 실패해도 나머지 진행, 실패는 `errors[]` 한국어 메모). 스케줄러(`scheduler.ts`)에 일일 크론 + **부팅 시 스냅샷 없거나 20h↑ 오래되면 즉시 수집**.
  ...
  - **미국 순유동성 카드**(`liquidity.ts`, 맨 아래 전폭): 순유동성 = 연준자산(WALCL) − 역레포(RRPONTSYD) − TGA(WTREGEN), 지급준비금(WRESBAL) 오버레이 2선. 소스=**FRED `fredgraph.csv?id=WALCL,RRPONTSYD,WTREGEN,WRESBAL`**(API키 불필요, 다중 시리즈 CSV). ⚠️**단위 주의**: WALCL/WTREGEN/WRESBAL=백만$, RRPONTSYD=**십억$** → 전부 $조로 정규화(`TO_TRILLIONS`). WALCL/TGA/WRESBAL은 **주간**(수 기준·목 공표), RRP만 일별 → 순유동성은 WALCL 주간 날짜에서 계산(RRP는 그 날짜 이하 최근값 carry-forward). 현재값+4주변화(임펄스)+지급준비금/RRP/TGA 레벨 표시. **기준선 없음·RefControls 없음**(임계 없는 지표), "주간·후행·매매신호 아님" 배지 + 경고문(2023 AI랠리 때 지수와 반대로 감). ⚠️**egress**: `fred.stlouisfed.org`가 샌드박스 허용목록에 없어 이 세션 검증 불가(CONNECT 403) — **Railway 프로덕션은 아웃바운드 개방이라 동작 예상**하나 실데이터는 배포 후 확인 필요. 데모 `SAMPLE_MARKET`에 더미 추가함. 파서 로직은 합성 CSV로 단위검증 완료.
  - ▶ **남은 개선(선택)**: 장중 ADR 더 자주(추가 크론), 기간 토글(1M/3M/1Y), F&G 색구간 배경, 다이제스트에 시황 한 줄 등.
  - ▶ **다음 세션 할 일 — 코스피/코스닥 신용잔고 카드 추가**(사용자 요청): 시황분석에 한 칸 더(신용거래융자 잔고; 코스피/코스닥 따로 제공되면 따로, 합산만 있으면 합산). ⚠️**현재 egress 허용목록에 소스가 없어 막힘**(`Host not in allowlist` 403; 이 세션에선 검증 불가). 사용자가 **`data.krx.co.kr`·`freesis.kofia.or.kr`·`finance.naver.com`·`api.stock.naver.com`·`m.stock.naver.com`** 세 소스를 허용목록에 추가하기로 함(egress 변경은 **새 세션부터 적용**). 다음 세션 절차: ① 세 소스 reachable 확인(curl) → ② **가장 효율적인 곳 선택**(권장 후보=KRX `data.krx.co.kr/comm/bldAttendant/getJsonData.cmd` POST, `bld=...` 신용거래융자 잔고 통계, Referer/UA 헤더 필요; OTP는 CSV다운로드만 필요하고 JSON엔 불필요) → ③ ADR과 동일 패턴으로 구현: `src/server/market/credit.ts`(현재값+1년 히스토리), `shared/market.ts`에 `credit:{kospi,kosdaq}` + `history.creditKospi/creditKosdaq` 추가, `index.ts` 병렬·tolerant 합류, 클라 `MarketPage`에 카드(라인차트, 기준선) 한 칸. KRX가 막히거나 비효율이면 KOFIA/네이버로 폴백. 데모모드 `SAMPLE_MARKET`에도 더미 추가.

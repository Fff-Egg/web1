...
- **검증**: 전체 276개 테스트·타입 검사·프로덕션 빌드 통과. 3개/4개/11개 경계, 원문 보존, 요청 0회, 중복 집계, RSS 유지, 기존 미분석 전환·완료 캐시 보존, 생략/실패 분리를 검증했다. 저장한 #83479 응답으로 오프라인 재현 시 링크 11개·외부 요청 0회·113,158→10,253자를 확인했다(운영 글은 변경하지 않음). 운영 배포 상태는 최종 확인 후 보고한다. 유료 모델을 호출하는 검증은 하지 않는다. health feature `post-reference-link-limit`.

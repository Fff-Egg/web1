...
## 최신 인수인계 — 2026-09-22 참고 링크 3개 초과 시 연결 자료 생략

- **사용자 기준**: 원래 X·텔레그램 게시글의 참고 링크가 4개 이상이면 연결 자료를 모두 생략하고 원 게시글 전체만 분석한다. 3개 이하는 기존대로 수집한다. 연결 기사 내부의 링크 수나 처음 3개 선택 기준이 아니다.
- **동작**: HTTP 요청 전에 원 게시글의 참조 주소를 센다. 동일 주소·알려진 추적 변형·자기 주소·확장 URL이 있는 경우의 `t.co` 중복을 제외한다. 미확인 단축 주소의 목적지는 요청 전에는 알 수 없어 별개로 센다. 원문·링크 주소를 보존하고 의도적 `skipped` 상태와 `linkExpansion` 사유를 표시한다. RSS 원문 수집은 유지한다.
- **기존 글**: 저장된 `sourceBody`가 있는 미분석 글만 다음 준비 때 무과금으로 새 범위로 전환한다. 해당 확장 입력의 읽기 캐시는 새 입력과 달라 무효화한다. 완료 분석·본문·캐시는 자동 변경하지 않으며 명시적 원문 갱신에는 새 규칙을 적용한다. 원래 본문이 없는 과거 글은 임의 절단하지 않는다. 이미 원글만 남아 있던 과거 글은 수집 설명과 캐시도 유지해 같은 본문을 재요약하지 않는다. 보류·전역 대기·모델·Thinking·운영 시각은 바꾸지 않는다.
- **실측 근거**: #83479 원글 10,253자 → 확장 113,158자. 추가 4구간 중 EU 우주 경제 문서가 약 98,700자로 대부분이었다. 원문에 참고 링크 11개가 있어 새 규칙에서는 연결 요청 0회다. 세부 목록과 정책은 `docs/FULL_ARTICLE_READING.md`.
- **검증**: 전체 276개 테스트·타입 검사·프로덕션 빌드 통과. 3개/4개/11개 경계, 원문 보존, 요청 0회, 중복 집계, RSS 유지, 기존 미분석 전환·완료 캐시 보존, 생략/실패 분리를 검증했다. 저장한 #83479 응답으로 오프라인 재현 시 링크 11개·외부 요청 0회·113,158→10,253자를 확인했다(운영 글은 변경하지 않음). 운영 `f946dac`의 Railway 배포 성공, 공개 health의 동일 revision과 `post-reference-link-limit` 기능을 확인했다. 유료 모델 검증 호출은 하지 않았다.

## 2026-09-22 HTTP 400 화면 추가 확인

- 사용자 화면은 글 #82829, 텔레그램 「중화권 마감 시황 (26.09.15, 화)」의 요청 기록과 일치했다. 저장 본문/제공 본문 모두 710자, 연결 원문 0건이다. 13:07:12 KST 선별 호출은 404ms·HTTP 400·`content_policy`, 입력/출력 토큰 미수신이며 이후 최신 호출은 없다. 운영 Settings에서 해당 글의 자동 재시도 보류를 확인했다. 이번 조회는 읽기 전용이며 재실행/보류 해제는 하지 않았다.
- 오전 09:56:17·09:41:52·09:26:33·08:56:27 호출은 오류 분류가 null이다. 당시 코드는 제공자 응답 상세를 보존하지 않았고 `882808a`에서 분류 저장을 도입했으므로 과거 오류를 모두 같은 정책 거절로 확정하지 않는다. 반복 호출은 영구 재시도 보류 도입 전 미완료 글을 다시 선택하던 경로와 일치한다.
- 현재 HTTP 400은 같은 입력/설정에서 즉시 보류하며 자동 배치에서 제외한다. 명시적 재시도 또는 입력/설정 변경은 다시 호출될 수 있으므로 영구 차단이라고 표현하지 않는다. 제공자 정책 거절을 토큰 한도 부족·연결 기사 확장 탓으로 설명하거나 특정 문구가 원인이라고 단정하지 않는다. 토큰 미수신은 무과금 증거가 아니다.

## 이전 인수인계 — 2026-09-22 밤 정상 응답 재압축 반복 제거 (배포·운영 단건 성공)
...
- **범위·검증**: 기존 키·원문·성공 캐시·학습 상한·정책 v2·분석 지문을 유지하며 기존 보류를 자동 해제하지 않는다. 384KiB는 글 하나의 준비 메모 예산이고 전체 프롬프트 토큰 계수나 다이제스트 최종 입력 전체의 한도 보장은 아니다. 최종 합성의 전역 예산/글 내부 맵 분할 재설계는 이번 범위 밖이다. 운영 07/14시, 자동 분석 10~13·15~19시 대기, 피드 정리 조건은 변경하지 않는다. 새 수정은 전체 **266개 테스트·타입 검사·프로덕션 빌드 통과**. 운영 `e8ee7ce`의 Railway 배포 성공과 공개 health revision을 확인했고, 비인증 진단 조회는 HTTP 401로 차단됐다. 이전 재압축 보류 글 3건을 명시적으로 재실행해 화면의 분석 완료와 사용량을 확인했다. 전체 읽기 추가 호출은 0회, 선별은 글마다 성공 1회였으며 출력은 각각 955·619·773토큰이었다. 3건 합계의 화면 비혼잡 단가 참고 비용 증가는 약 $0.0092였다. 첫 단건은 입력 캐시 적중 3,456·미적중 22,061과 출력 955토큰으로 참고 비용 $0.003892518이었다. 기존 메모 재사용과 확인한 3건의 완료 결과이며 모든 보류 글의 성공·요약 품질을 보장하지 않는다. 상세 `docs/FULL_ARTICLE_READING.md`·`docs/LLM_USAGE.md`.

- **남은 운영 확인**: 완료를 확인한 것은 위 3건이다. 추가 한 건은 단건 실행을 시작했지만 브라우저 연결 문제로 최종 결과를 확인하지 못했다. 접근이 복구되면 먼저 해당 실행 결과를 조회하고, 나머지 읽기 보류는 상태를 확인한 뒤 필요한 글만 명시적으로 재개한다. `content_policy` 보류 8건은 그대로 유지했다. 전체 보류가 해결됐다고 기록하지 않는다.

...
- **시황분석**(tab 1, `src/client/pages/MarketPage.tsx`): 시장 지표 대시보드 **✅ 구현 완료**. 하루 1회 배치 수집(기본 07시 KST `MARKET_HOUR`) → `settings` KV(`key="marketSnapshot"`, 마이그레이션 불필요)에 JSON 스냅샷 저장. tRPC `market.latest`(저장본)·`market.refresh`('지금 갱신' 버튼=즉시 재수집). 서버 수집기 = **`src/server/market/`**: `cnn.ts`/`tradingview.ts`/`adr.ts`/`index.ts`(병렬·소스별 tolerant — 한 곳 실패해도 나머지 진행, 실패는 `errors[]` 한국어 메모). 스케줄러(`scheduler.ts`)에 일일 크론 + **부팅 시 스냅샷 없거나 20h↑ 오래되면 즉시 수집**.
  ...
  - **캐피출레이션(투매) 바닥 감지 패널**(맨 아래, `CapitulationPanel.tsx`+`capitulation.ts`): 국내 지수 4신호 O/X + 3/4 충족 시 "분할 예비대" 안내(매수신호 아님 명시). ① 신용잔고(유가+코스닥 합산, 기존 데이터) 피크 대비 −8%+10일 −3%+지수 동반↓ / ② 미수 반대매매 비중 스파이크(1년 95%ile) 후 피크아웃 / ③ VKOSPI 95%ile 후 꺾임(−20% or 3일↓) / ④ 60일 이격도 과매도(1년 하위5% or ≤92). 신호 계산은 **전부 클라(stored history에서)**. 신호별 데이터 없으면 O/X 대신 '–'(관대). 합성데이터 검증 OK(판별력 있음). ⚠️**새 데이터 소스 3개는 Railway 실검증 대기**: 코스피종가·VKOSPI = TradingView `KRX:KOSPI`/`KRX:KSVKOSPI`(심볼 확실도 KOSPI>VKOSPI, 폴백 목록 있음), 반대매매 비중 = KOFIA `STATSCU0100000060`(증시자금추이) — **컬럼(TMPV?) 매핑 미검증**이라 `forcedLiq.ts`가 첫 행 키를 콘솔 로그 + 0~30% 플로저빌리티 게이트(틀리면 빈값→'데이터 없음'). 배포 후 '지금 갱신' 로그로 VKOSPI 심볼·반대매매 컬럼 확정 필요.

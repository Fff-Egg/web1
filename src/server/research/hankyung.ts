/**
 * 한경 컨센서스 (consensus.hankyung.com) — daily broker research list, used alongside
 * Naver. The list is server-rendered HTML whose columns are
 * 작성일 · 분류 · 제목 · 적정가격(TP) · 투자의견 · 작성자/제공출처(증권사) · 첨부(PDF),
 * so TP/opinion come straight from the list (no detail fetch). We keep only 기업·산업.
 *
 * Tolerant regex + header-mapped parsing (the project has no DOM lib — see rss.ts).
 * ⚠ This sandbox blocks consensus.hankyung.com (egress); runs live on Railway.
 */
...
/** Fetch 한경 reports with 작성일 in [sdate, edate] (YYYY-MM-DD), 기업·산업만. */
export async function fetchHankyung(sdate: string, edate: string, maxPages = 12): Promise<ParsedReport[]> {

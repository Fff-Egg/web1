...
- **글별 처리 내역**: 재시도 관리의 처리 내역에서 저장 본문 전체·제공 본문 크기·수집 범위·완료 요약 표본·복구 보류 원인·학습된 요청 입력 상한·최근 API 호출 최대 30건을 조회한다. 부모/자식 캐시를 함께 세므로 완료 캐시 합계를 고유 원문 처리량으로 보지 않는다. 기존 인증으로 보호되는 읽기 전용 조회이며 AI 호출·재수집·보류 해제를 하지 않는다.
- **단건 검증**: 최근 7일 반복 실패 글의 **1건 다시 분석**에서 특정 미분석 글 하나만 확인 후 즉시 분석한다. 유료 호출이 생길 수 있고 기존 완료 구간은 재사용한다. 실행 잠금 중에는 중복 실행하지 않으며 주변 대기 글 전체를 처리하지 않는다. 전체 분석 대기는 유지하고 선택한 한 글만 우회한다. 기존 Basic Auth로 보호되며 일반 제한 해제와 별개다.
